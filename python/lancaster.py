...
import re # the re module is used to parse the stemming rules
...
class Lancaster:
    ...
    def parseRules(self, rule_tuple):
        ...
        for rule in rule_tuple:
            if not valid_rule.match(rule):
                message = "The rule %s is invalid" % rule
                raise InvalidRuleError(message)
            first_letter = rule[0:1]
            if self.rule_dictionary.has_key(first_letter):
    ...
    def __doStemming(self, word, intact_word):
        ...
        while continue_stemming == True:
            ...
            # that last letter
            if (last_letter_position < 0 or
                not self.rule_dictionary.has_key(word[last_letter_position])):
    ...
    def __isAcceptable(self, word, remove_total):
        ...
        # characters long (including one vowel) to be stemmed
            ...
            if self.__isVowel(word[1]):
                word_is_acceptable = True
            elif self.__isVowel(word[2]):
                word_is_acceptable = True            
    ...
    def __isVowel(self, letter):
        ...
        """
        # For the purposes of this stemmer, 'y' is a vowel
    ...
    def __applyRule(self, word, remove_total, append_string):
        ...
        if append_string: word = word + append_string
        return word
...
def demo():
    ...
    Paice, Chris D. "Another Stemmer." ACM SIGIR Forum 24.3
    (1990): 56-61.
    """
...
## Call demo() if this module is invoked directly.
