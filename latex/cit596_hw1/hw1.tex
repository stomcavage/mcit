...
an odd number of `a's and ends with a b}\}$.
...
Create a DFA that accepts the language $\{\omega \where \omega \text{ has
an even length and an odd number of `a's}\}$.
...
not contain ab nor ba}\}$.

\begin{figure}[h!]
	\centering
	\subfigure[A accepts $\{\omega \where \omega \text{ contains ab or ba}\}$] {
		\includegraphics[height=2.0in]{1_5_c_a.png}
	}
	\subfigure[DFA that accepts $\not{A}$] 
	{
